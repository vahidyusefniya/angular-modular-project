...
export class AddComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.layoutService.setBreadcrumbs([
      ...
      },
      {
        url: `/branches/${this.branchId}/gateway-lists/`,
        deActive: false,
        label: dictionary.NewGatewayList,
      },
    ]);
    this.getProviders();
  }
  ...
  onCreateGatewayList() {
    this.loadingService.present();
    const request = new CreateGatewayListRequest();
    ...
    this.gatewayListClient.create(this.branchId!, request).subscribe({
      next: (res) => {
        this.loadingService.dismiss();
        this.notificationService.showSuccessNotification(
          `Gateway List ${request.name} successfully created.`
        );
